...
```{r}
naive <- coxph(Surv(futime, fustat) ~ transplant + age, data=jasa)
naive
```
...
|id | tstart | tstop | death | age |  transplant |
|:---|:-----|:------|:-----|:----|:---------|
| 1 | .   | . | . | . |
...
```{r, echo=TRUE}
fit <- coxph(Surv(tstart, tstop, death) ~ age + transplant, 
             data=sdata)
fit
```
...
* The data `r nrow(pbcseq)` repeated laboratory values + baseline variables 
...
dn3 <- tmerge(dn2, dnase, 
             no.risk=event(pmin(ivstop+6,end.tm)), id=id)
dn3[dn3$id==129,]
```
...
```{r, echo=TRUE}
newdnase <- tmerge(dn4, dn4, enum=cumtdc(dn4$tstart), id=id)
```
...
## Maybe more tweaking is needed?

```{r, echo=TRUE}
table(dnase$ivstart==dnase$end.tm)

dnase %>% filter(ivstart==end.tm) %>%
  select(id,fev,ivstart,ivstop,end.tm)
```

## 
```{r, echo=TRUE}
newdnase %>% filter(id %in% c(212, 486, 535))
dnase %>% filter(id %in% c(212, 486, 535)) %>%
  select(id,fev, ivstart, ivstop, end.tm)
```
...
## Example: MGUS data - Competing Risk
...
## Example: MGUS data - Aalen-Johansen estimate
